namespace Navy.UI
{
    public class TextBox : CanvasElement
    {
        public TextBox(Rectangle RawRect, Texture2D backgroundTex, SpriteFont Font = null)
        {
        ...
        protected override void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (!string.IsNullOrEmpty(Text))
            {
                ...
                {
                    float scale = 1;
                    if (GetTextSize().X * Scale.X > Rect.Width - TextPadding.X * 2)
                    {
                    ...
                    else if (GetTextSize().Y * Scale.Y > Rect.Height - TextPadding.Y * 2)
                    {
                        scale = (Rect.Height - TextPadding.X * 2) / GetTextSize().Y;
